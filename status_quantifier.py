...
for year in years:
    ...
    with open(person_status_path) as person_status_csv:
        ...
        counted_csv = [["Nome", "Arquivados", "Retirados", "Tramitando", "Total"]]
        for row in name_reader:
            total, arquivados, retirados, tramitando = [0] * 4
            for status in row['Status'].rsplit(','):
                if (status == 'A Proposicao procurada nao existe'):
                    ...
                    tramitando += 1
                    total += 1
            counted_csv.append([row['Nome'], arquivados, retirados, tramitando, total])
